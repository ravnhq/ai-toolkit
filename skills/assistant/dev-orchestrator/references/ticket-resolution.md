...
When Phase 0 resolved a ticket **and** Phase 1 sets the worktree flag, the orchestrator should pass the ticket id into `setup-worktree.sh` so the feature branch name embeds it. Replace the default auto-derived slug with `<tracker-prefix>/<ticket-id>-<task-slug>-<unix-seconds>`:

```bash
bash skills/assistant/dev-orchestrator/scripts/setup-worktree.sh \
  --branch "agent/ABC-42-auth-refactor-$(date +%s)" \
  --task "<task body>"
```

This makes the feature branch immediately traceable to its source ticket in commit history, PR titles, and Linear's branch-link autodetection — without changing the script signature (`--branch` already exists, Phase 0 just supplies a richer value).
...
**Phase 1 sees** three distinct acceptance criteria + three tracker labels → classifies as **three soft-sequenced tasks** with the worktree flag still on (carried over from `--worktree` in the original prompt).

**Phase 2 — Layout C** is presented (single isolated branch, sequential execution) with the ticket id included in the options echo: `Worktree options: branch=agent/ABC-42-auth-refactor-<unix>, base=main, validate=on, merge=on, keep-branch=off`.

**Phase 3 Mode 3** provisions the worktree on that branch and runs the three subtasks sequentially, each commit linked back to ABC-42 by branch naming.

**Phase 4.5** merges the branch back to `main`. The push creates commits whose subject lines reference ABC-42, so Linear's GitHub integration auto-updates the ticket.

**Phase 5** hands off to `qa-orchestrator` with the ticket URL added to the scope blurb: `goal: implement ABC-42 end-to-end | ticket: https://linear.app/team/issue/ABC-42 | landed: <code-touching summaries>`. QA scopes its plan against the merged base.

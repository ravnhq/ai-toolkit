...
Swift 6.2 introduces "Approachable Concurrency" - a set of features that make Swift Concurrency easier to adopt incrementally. The philosophy follows Swift's "progressive disclosure" principle:

1. **Level 1**: Single-threaded sequential code (no data races possible)
2. **Level 2**: Async code with basic suspension (still single-threaded default)
3. **Level 3**: Parallel code with compiler-enforced data race safety

### When Both Modes Are Enabled

With **Default Actor Isolation** set to `@MainActor` AND **Approachable Concurrency** enabled, code runs on the main thread EXCEPT in these three cases:

1. **`@concurrent` attribute** - Explicitly requests background execution
2. **Any actor other than MainActor** - Custom actors have their own serial executor
3. **Detached tasks** - `Task.detached` runs independently of caller context

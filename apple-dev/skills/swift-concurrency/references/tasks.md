...
### Common Gotcha: Nested Tasks Don't Cascade Cancellation

**Important misconception**: Many developers assume that nesting `Task {}` blocks creates a parent-child relationship for cancellation. **This is wrong.**

```swift
let parentTask = Task {
    Task {
        // This is NOT a child task!
        // Cancelling parentTask does NOT cancel this task
        try await expensiveWork()
    }
}

parentTask.cancel() // Inner task continues running!
```

**True child tasks** (cancellation cascades) are created ONLY by:
- `async let`
- Task groups (`withTaskGroup`, `withThrowingTaskGroup`)

```swift
// Cancellation DOES cascade here:
let parent = Task {
    async let child1 = work(1) // True child
    async let child2 = work(2) // True child
    return try await [child1, child2]
}

parent.cancel() // Both child1 and child2 are notified
```

### What Nested Tasks DO Inherit

While nested `Task {}` blocks don't inherit cancellation, they DO inherit:
- **Actor isolation context** - Runs on same actor as parent
- **Priority** (unless detached)
...
func example() {
    Task {
        ...
        Task {
            // Also runs on MainActor (inherited)
        }
    }
}
```

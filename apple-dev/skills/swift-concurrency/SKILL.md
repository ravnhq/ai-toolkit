...
3. **Managing async operations?**
   ...
   - Complex stream operations (debounce, merge, combineLatest) → `references/async-algorithms.md`
...
8. **Building distributed systems?**
   - Cross-process communication → `references/distributed-actors.md`
...
- Need to debounce, throttle, or combine async streams
  - Use `references/async-algorithms.md` for swift-async-algorithms package
- Task cancellation not propagating to nested tasks
  - This is expected! Use `references/tasks.md` (gotcha section) - only `async let` and task groups create true child tasks
- Building peer-to-peer or distributed systems in Swift
  - Use `references/distributed-actors.md` for cross-process actor isolation
...
- **`threading.md`** - Thread/task relationship, suspension points, isolation domains, nonisolated, Swift 6.2 approachable concurrency
...
- **`async-algorithms.md`** - swift-async-algorithms package: debounce, throttle, merge, combineLatest, chunked
- **`distributed-actors.md`** - Distributed actors for cross-process communication, actor systems
